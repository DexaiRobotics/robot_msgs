...
#ifndef __robot_msgs_lcmt_polynomial_matrix_hpp__
#define __robot_msgs_lcmt_polynomial_matrix_hpp__
...
class lcmt_polynomial_matrix
{
    public:
        ...
        std::vector< std::vector< robot_msgs::lcmt_polynomial > > polynomials;
...
uint64_t lcmt_polynomial_matrix::_computeHash(const __lcm_hash_ptr *p)
{
    ...
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == lcmt_polynomial_matrix::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, lcmt_polynomial_matrix::getHash };

    uint64_t hash = 0xb7c3cad59f376c2dLL +
         robot_msgs::lcmt_polynomial::_computeHash(&cp);
